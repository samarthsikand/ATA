...
public class RuntimeATA {
	...
	public void runTheTuples(List<String[]> tuples) {
		System.out.println("The size of tuple are: "+tuples.size());
		...
		try {
			if(tuple[1].equalsIgnoreCase("enter")) {
				...
				if(tuple[0].equalsIgnoreCase("button")) {
					...
					wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.xpath("//a[contains(text(),'"+tuple[2]+"')]")));
					...
					String[] prefixSelect = tuple[2].split("/");
					if(prefixSelect.length != 1) {
						ele = driver.findElements(By.xpath("//*[text()='"+prefixSelect[0]+"']/following::select[1] | //*[contains(text(),'"+prefixSelect[0]+"')]/descendant::select[1]"));
					} else {
...
//					driver.switchTo().activeElement().sendKeys("test.html");
			...
			System.out.println("The error following is because of: " + tuple[2]);
			e.printStackTrace();
		}
		}
